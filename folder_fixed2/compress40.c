...
void decompress_apply(int col, int row, A2 array, A2Methods_Object *elem, void *cl) {
        ...
        if (row % 2 == 1 && col % 2 == 1) {
                ...
                fprintf(stderr, "Y1 is, %f\n", *(float*)UArray_at(Y_values, 0));
                fprintf(stderr, "Y2 is, %f\n", *(float*)UArray_at(Y_values, 1));
                fprintf(stderr, "Y3 is, %f\n", *(float*)UArray_at(Y_values, 2));
                fprintf(stderr, "Y4 is, %f\n", *(float*)UArray_at(Y_values, 3));
...
extern void decompress40(FILE *input)
{
        ...
        A2Methods_T methods = uarray2_methods_blocked;
        A2 pixels = methods->new_with_blocksize(width, height, sizeof(struct Pnm_rgb), 2);
